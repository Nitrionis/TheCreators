#include "Scene/SceneRenderer/VulkanSharedData/VulkanSharedData.h"
...
void Vulkan::ShowIntermediateImage(uint32_t index) {
	...
	device.SetImageBarrier(image.intermediate[index].obj,
...
void Vulkan::CreateIntermediateImages() {
	...
	for (int i = 0; i < 3; i++) {
		...
		viewInfo.image      = image.intermediate[i].obj;
